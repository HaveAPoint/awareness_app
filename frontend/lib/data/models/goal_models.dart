...
class ObjectiveModel {
  ...
  // 从数据库记录创建
  factory ObjectiveModel.fromDb(Map<String, dynamic> row, {List<KeyResultModel>? keyResults}) {
    return ObjectiveModel(
      ...
      calculatedProgress: (row['calculatedProgress'] as num?)?.toDouble() ?? 0.0,
      keyResults: keyResults ?? [],
    );
  }

  // 复制并更新 keyResults
  ObjectiveModel copyWith({List<KeyResultModel>? keyResults}) {
    return ObjectiveModel(
      id: id,
      title: title,
